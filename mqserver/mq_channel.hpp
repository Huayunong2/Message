...
#include "../mqthird/muduo/protoc/codec.h"
#include "../mqthird/muduo/protoc/dispatcher.h"
...
namespace MQ
{
    ...
    public:
        ...
        ~Channel()
        {
            ...
            DLOG("delete channel %s", _cid.c_str());
        }
        ...
        void basicPublish(const basicPublishRequestPtr &req)
        {
            ...
            for (auto &binding : mqbm)
            {
                if (Router::route(ep->type, routing_key, binding.second->binding_key))
                {
                    ...
                    // 4.向线程池中添加一个消息消费任务(向指定队列的订阅者推送消息——线程池完成)
    ...
    private:
        ...
        void consume(const std::string &qname)
        {
            ...
            if (mp.get() == nullptr)
            {
                DLOG("执行消费任务时，队列 %s 没有消息!", qname.c_str());
                return;
            }
            // 2.从队列订阅者中取出一个订阅者
